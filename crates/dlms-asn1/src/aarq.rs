    ...
    pub fn decode(data: &[u8]) -> Result<Self, BerError> {
        ...
        while inner.remaining() > 0 {
            ...
            match field_tag.number {
                0x00 | 0x01 => {  // Accept both tag 0 (standard) and tag 1 (some implementations)
                ...
                0x06 | 0x1E => {  // Accept context-6 (some impls) and context-30 (standard)
                    ...
                    user_info = Some(if raw_data.len() >= 2 && raw_data[0] == 0xA1 {
                        ...
                        crate::InitiateRequest {
                            ...
                            proposed_conformance: crate::ConformanceBlock::from_bytes(&raw_data[1..4])
...
mod python_compat_tests {
    ...
    fn test_decode_python_dlms_cosem_aarq() {
        ...
        let aarq_bytes: Vec<u8> = vec![
            0x60, 0x29, 0xA1, 0x09, 0x06, 0x07, 0x60, 0x85, 0x74, 0x05,
            0x08, 0x01, 0x01, 0xA6, 0x0A, 0x04, 0x08, 0x75, 0x74, 0x69,
            0x0B, 0x1A, 0x70, 0x5C, 0xA8, 0xBE, 0x10, 0x04, 0x0E, 0x01,
            0x00, 0x00, 0x00, 0x06, 0x5F, 0x1F, 0x04, 0x00, 0x20, 0x52,
            0x5F, 0xFF, 0xFF,
        ];
